...
# Extract month name for filtering
...
if selected_option == 'All options':
    ...
    graph_all=alt.Chart(df_selected).mark_line().encode(
        ...
        tooltip=['time:T', 'value:Q']).properties( width=800,height=400,title= selected_option).interactive()
...
#Creating plot of each column
    ...
    color=alt.Color('variable:N', title='Variable'),
    tooltip=['time:T', 'value:Q']).properties(title= selected_option).interactive()
